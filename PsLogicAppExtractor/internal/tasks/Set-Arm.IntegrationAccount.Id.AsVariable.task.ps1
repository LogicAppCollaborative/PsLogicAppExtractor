...
Creates an Arm variable: integrationAccountId
-Set the value to: [format('/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Logic/integrationAccounts/{2}',subscription().subscriptionId,resourceGroup().name,variables('integrationAccount'))]
Sets the value of the integrationAccount.Id: [variables('integrationAccountId')]
"@
    Alias       = "Arm.Set-Arm.IntegrationAccount.Id.AsVariable"
}

Task -Name "Set-Arm.IntegrationAccount.Id.AsVariable" @parm -Action {
    ...
    if ($armObj.resources[0].properties.integrationAccount.id) {
        ...
        $armObj.resources[0].properties.integrationAccount.id = "[variables('integrationAccountId')]"
        ...
        $armObj = Add-ArmVariable -InputObject $armObj -Name "integrationAccountId" -Value "[format('/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Logic/integrationAccounts/{2}',subscription().subscriptionId,resourceGroup().name,variables('integrationAccount'))]"
    }
