Describe 'Testing Sort-Arm.Variable' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "ConvertTo-Arm", "Set-Arm.Tags.AsVariable", "Sort-Arm.Variable"
    ...
    It "First parameter is tag_CostCenter" {
        $($armObj.variables.PsObject.Properties)[0].Name | Should -BeExactly "tag_CostCenter"
    }
    
    It "Second parameter is tag_Department" {
        $($armObj.variables.PsObject.Properties)[1].Name | Should -BeExactly "tag_Department"
    }
