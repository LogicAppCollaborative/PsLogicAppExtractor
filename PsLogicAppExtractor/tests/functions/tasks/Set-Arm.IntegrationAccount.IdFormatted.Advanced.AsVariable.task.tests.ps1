Describe 'Set-Arm.IntegrationAccount.IdFormatted.Advanced.AsVariable' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "Set-Raw.ApiVersion", "ConvertTo-Arm", "Set-Arm.IntegrationAccount.IdFormatted.Advanced.AsVariable"
    ...
    It 'Should be "[format(...)]" in the resources[0].properties.integrationAccount.id property' {
        $armObj.resources[0].properties.integrationAccount.id | Should -BeExactly "[format('/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Logic/integrationAccounts/{2}', subscription().subscriptionId,variables('integrationAccountResourceGroup'),variables('integrationAccount'))]"
    }
    ...
    It "Should have a variables.integrationAccountResourceGroup property" {
        $armObj.variables.integrationAccountResourceGroup | Should -Not -Be $null
    }

    It "Should be 'rgTest' in the variables.integrationAccountResourceGroup property" {
        $armObj.variables.integrationAccountResourceGroup | Should -BeExactly "rgTest"
    }
