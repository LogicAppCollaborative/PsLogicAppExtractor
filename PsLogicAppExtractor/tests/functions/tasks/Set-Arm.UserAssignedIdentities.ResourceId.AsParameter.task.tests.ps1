Describe 'Testing Set-Arm.UserAssignedIdentities.ResourceId.AsParameter' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "Set-Raw.ApiVersion", "ConvertTo-Arm", "Set-Arm.UserAssignedIdentities.ResourceId.AsParameter"
    ...
    It "Should have a parameters.userAssignedIdentityName property" {
        $armObj.parameters.userAssignedIdentityName | Should -Not -Be $null
    }

    It 'Should be "Test-ManagedIdentity" in the parameters.userAssignedIdentityName.defaultValue property' {
        $armObj.parameters.userAssignedIdentityName.defaultValue | Should -BeExactly "Test-ManagedIdentity"
    }

    It 'Should match "[resourceId(...)]" in the LogicApp json' {
        $raw | Should -match "[resourceId('Microsoft.ManagedIdentity/userAssignedIdentities/', parameters('UserAssignedIdentityName'))]"
    }
