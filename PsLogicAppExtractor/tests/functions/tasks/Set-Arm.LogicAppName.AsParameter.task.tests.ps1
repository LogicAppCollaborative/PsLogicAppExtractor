Describe 'Testing Set-Arm.LogicApp.Name.AsParameter' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "Set-Raw.ApiVersion", "ConvertTo-Arm", "Set-Arm.LogicApp.Name.AsParameter"
    ...
    It "Should have a parameters.logicAppName property" {
        $armObj.parameters.logicAppName | Should -Not -Be $null
    }

    It 'Should be "LA-TEST-Exporter" in the parameters.logicAppName.defaultValue property' {
        $armObj.parameters.logicAppName.defaultValue | Should -BeExactly "LA-TEST-Exporter"
    }

    It 'Should be "[parameters(''logicAppName'')]" in the resources[0].name property' {
        $armObj.resources[0].name | Should -BeExactly "[parameters('logicAppName')]"
    }
