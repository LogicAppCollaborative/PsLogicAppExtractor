Describe 'Testing Set-Arm.Trigger.ApiConnection.Recurrence.AsParameter' {
    ...
    It 'Should be "[parameters(''trigger_Frequency'')]" in the properties.definition.triggers.recurrence.frequency property' {
        @($armObj.resources[0].properties.definition.triggers.PsObject.Properties)[0].Value.recurrence.frequency | Should -BeExactly "[parameters('trigger_Frequency')]"
    }

    It 'Should be "[parameters(''trigger_Interval'')]" in the properties.definition.triggers.recurrence.interval property' {
