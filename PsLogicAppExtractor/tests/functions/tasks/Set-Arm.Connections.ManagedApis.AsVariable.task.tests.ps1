Describe 'Set-Arm.Connections.ManagedApis.AsVariable' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "Set-Raw.ApiVersion", "ConvertTo-Arm", "Set-Arm.Connections.ManagedApis.AsVariable"
    ...
    It 'Should be "[resourceId(...)]" in the resources[0].properties.parameters.$connections.value.servicebus.connectionId property' {
        $armObj.resources[0].properties.parameters.'$connections'.value.servicebus.connectionId | Should -BeExactly "[resourceId('Microsoft.Web/connections', variables('connection_servicebus_id'))]"
    }

    It 'Should be "[parameters(...)]" in the resources[0].properties.parameters.$connections.value.servicebus.connectionName property' {
        $armObj.resources[0].properties.parameters.'$connections'.value.servicebus.connectionName | Should -BeExactly "[variables('connection_servicebus_id')]"
    }

    It "Should have a variables.connection_servicebus_id property" {
        $armObj.variables.connection_servicebus_id | Should -Not -Be $null
    }

    It "Should be 'servicebus' in the variables.connection_servicebus_id property" {
        $armObj.variables.connection_servicebus_id | Should -BeExactly "servicebus"
    }
