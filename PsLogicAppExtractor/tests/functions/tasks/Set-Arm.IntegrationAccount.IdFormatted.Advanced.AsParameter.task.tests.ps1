Describe 'Set-Arm.IntegrationAccount.IdFormatted.Advanced.AsParameter' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "Set-Raw.ApiVersion", "ConvertTo-Arm", "Set-Arm.IntegrationAccount.IdFormatted.Advanced.AsParameter"
    ...
    It 'Should be "[format(...)]" in the resources[0].properties.integrationAccount.id property' {
        $armObj.resources[0].properties.integrationAccount.id | Should -BeExactly "[format('/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Logic/integrationAccounts/{2}', subscription().subscriptionId, parameters('integrationAccountResourceGroup'), parameters('integrationAccount'))]"
    }

    It "Should have a parameters.integrationAccount property" {
        $armObj.parameters.integrationAccount | Should -Not -Be $null
    }

    It "Should be 'Test-IntegrationAccount' in the parameters.integrationAccount.defaultValue property" {
        $armObj.parameters.integrationAccount.defaultValue | Should -BeExactly "Test-IntegrationAccount"
    }

    It "Should have a parameters.integrationAccountResourceGroup property" {
        $armObj.parameters.integrationAccountResourceGroup | Should -Not -Be $null
    }

    It "Should be 'rgTest' in the parameters.integrationAccountResourceGroup.defaultValue property" {
        $armObj.parameters.integrationAccountResourceGroup.defaultValue | Should -BeExactly "rgTest"
    }
