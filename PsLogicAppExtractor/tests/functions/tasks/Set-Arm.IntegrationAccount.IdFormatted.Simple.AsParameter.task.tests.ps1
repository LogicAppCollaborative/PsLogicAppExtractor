Describe 'Set-Arm.IntegrationAccount.IdFormatted.Simple.AsParameter' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "Set-Raw.ApiVersion", "ConvertTo-Arm", "Set-Arm.IntegrationAccount.IdFormatted.Simple.AsParameter"
    ...
    It 'Should be "[format(...)]" in the resources[0].properties.integrationAccount.id property' {
        $armObj.resources[0].properties.integrationAccount.id | Should -BeExactly "[format('/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Logic/integrationAccounts/{2}', subscription().subscriptionId, resourceGroup().name, parameters('integrationAccount'))]"
    }
