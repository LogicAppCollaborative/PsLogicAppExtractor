Describe 'Testing Set-Arm.Tags.AsVariable' {
    ...
    BeforeAll {
        ...
        Invoke-psake @parms -taskList "ConvertTo-Arm", "Set-Arm.Tags.AsVariable"
    ...
    It "Should be '[variables('tag_Department')]' in the tags.Department property" {
        $armObj.resources[0].tags.Department | Should -BeExactly "[variables('tag_Department')]"
    }

    It "Should be '[variables('tag_CostCenter')]' in the tags.CostCenter property" {
        $armObj.resources[0].tags.CostCenter | Should -BeExactly "[variables('tag_CostCenter')]"
    }

    It "Should be 'Unknown' in the variables.tag_Department property" {
        $armObj.variables.tag_Department | Should -BeExactly "Unknown"
    }

    It "Should be 'Unassigned' in the variables.tag_CostCenter property" {
        $armObj.variables.tag_CostCenter | Should -BeExactly "Unassigned"
    }
