...
$parentModule = 'PsLogicAppExtractor'
if (-not $ModuleName) { $ModuleName = 'PsLogicAppExtractor.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PsLogicAppExtractor'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PsLogicAppExtractor.psd1"
$prereqHash = @{
